...
public partial class RoxyFilemanService : IRoxyFilemanService
{
    ...
    /// <summary>
    /// Normalize directory path for database operations
    /// </summary>
    /// <param name="path">The path to normalize</param>
    /// <returns>Normalized path for database operations</returns>
    ...
    public IEnumerable<object> GetDirectoryList(string type)
    {
        ...
        {
            p = "/",
            f = _fileProvider.GetFiles(NormalizeDirectoryPath("/"), type).Count(), // Use normalized path for root directory
            d = 0
        } };
    ...
    {
        // Normalize root directory path for database operations
        var normalizedPath = NormalizeDirectoryPath(directoryPath);
        
        return _fileProvider.GetFiles(normalizedPath, type?.Trim('#'))
            .Select(f => new
            {
    ...
    public async Task UploadFilesAsync(string directoryPath, IEnumerable<IFormFile> files)
    {
        ...
        {
            await _fileProvider.SaveFileAsync(normalizedPath, formFile.FileName, formFile.ContentType, formFile.OpenReadStream());
        }
    }
