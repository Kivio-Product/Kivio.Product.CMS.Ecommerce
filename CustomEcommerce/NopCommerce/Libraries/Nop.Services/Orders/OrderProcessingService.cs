...
public partial class OrderProcessingService : IOrderProcessingService
{
    ...
    protected virtual async Task PrepareAndValidateShoppingCartAndCheckoutAttributesAsync(PlaceOrderContainer details, ProcessPaymentRequest processPaymentRequest, Currency currentCurrency)
    {
        ...
        if (!await ValidateMinOrderSubtotalAmountAsync(details.Cart, processPaymentRequest.StoreId))
        {
            var orderSettings = await _settingService.LoadSettingAsync<OrderSettings>(processPaymentRequest.StoreId);
            var minOrderSubtotalAmount = await _currencyService.ConvertFromPrimaryStoreCurrencyAsync(orderSettings.MinOrderSubtotalAmount, currentCurrency);
        ...
        if (!await ValidateMinOrderTotalAmountAsync(details.Cart, processPaymentRequest.StoreId))
        {
            var orderSettings = await _settingService.LoadSettingAsync<OrderSettings>(processPaymentRequest.StoreId);
            var minOrderTotalAmount = await _currencyService.ConvertFromPrimaryStoreCurrencyAsync(orderSettings.MinOrderTotalAmount, currentCurrency);
    ...
    /// <param name="storeId">Store identifier; pass null to use the current store settings</param>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains true - OK; false - minimum order sub-total amount is not reached
    /// </returns>
    public virtual async Task<bool> ValidateMinOrderSubtotalAmountAsync(IList<ShoppingCartItem> cart, int? storeId = null)
    {
        ArgumentNullException.ThrowIfNull(cart);

        //Get the order settings for the specific store or use the default
        var orderSettings = storeId.HasValue 
            ? await _settingService.LoadSettingAsync<OrderSettings>(storeId.Value)
            : _orderSettings;
        ...
        //subtotal
        var (_, _, subTotalWithoutDiscountBase, _, _) = await _orderTotalCalculationService.GetShoppingCartSubTotalAsync(cart, orderSettings.MinOrderSubtotalAmountIncludingTax);
    ...
    /// </returns>
    public virtual async Task<bool> ValidateMinOrderTotalAmountAsync(IList<ShoppingCartItem> cart, int? storeId = null)
    {
        ArgumentNullException.ThrowIfNull(cart);
