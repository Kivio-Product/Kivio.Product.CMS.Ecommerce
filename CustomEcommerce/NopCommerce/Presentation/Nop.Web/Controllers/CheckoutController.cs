...
public partial class CheckoutController : BasePublicController
{
    ...
    public virtual async Task<IActionResult> OpcCompleteRedirectionPayment()
    {
        try
        {
            // Verificar si la funcionalidad de pago en una sola página está habilitada
            if (!_orderSettings.OnePageCheckoutEnabled)
                return RedirectToRoute("Homepage");
            ...
            var paymentMethod = await _paymentPluginManager.LoadPluginBySystemNameAsync(order.PaymentMethodSystemName, customer, store.Id);
            if (paymentMethod == null || paymentMethod.PaymentMethodType != PaymentMethodType.Redirection)
                return RedirectToRoute("Homepage");

            // Asegurarse de que la orden se creó recientemente
            if ((DateTime.UtcNow - order.CreatedOnUtc).TotalMinutes > 3)
                return RedirectToRoute("Homepage");

            // Procesar el pago mediante redirección
            var postProcessPaymentRequest = new PostProcessPaymentRequest
            {
                Order = order
            };
            ...
            // Verificar si ya se realizó una redirección o un POST
            if (_webHelper.IsRequestBeingRedirected || _webHelper.IsPostBeingDone)
            {
                // Redirección o POST realizados, no agregar contenido adicional
                return new EmptyResult();
            }

            // Si no se realizó una redirección, redirigir al resumen de pedido completado
