...
public partial class CatalogModelFactory : ICatalogModelFactory
{
    ...
    public virtual async Task<CatalogProductsModel> PrepareAllProductsModelAsync(Category category, CatalogProductsCommand command)
    {
        ArgumentNullException.ThrowIfNull(category);
        ArgumentNullException.ThrowIfNull(command);
        ...
        var filteredSpecs = command.Specs is null ? null :
            filterableOptions.Where(fo => command.Specs.Contains(fo.Id)).ToList();
