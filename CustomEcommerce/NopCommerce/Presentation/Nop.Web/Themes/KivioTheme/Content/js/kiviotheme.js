...
$(function () {
  ...
  $(document).on("click", function (event) {
    if (
      userOverlay.hasClass("active") &&
      !userOverlay.is(event.target) &&
      userOverlay.has(event.target).length === 0 &&
      !myAccount.is(event.target) &&
      myAccount.has(event.target).length === 0
    ) {
...
function divShow() {
  ...
  var visibleTopHeader = Math.max(
    0,
    Math.min(headerTop + headerHeight, scrollTop + windowHeight) -
      Math.max(headerTop, scrollTop)
  );
  ...
  if (productDetail.length > 0) {
    ...
    if (right.outerHeight() > initialLeftHeight && visibleTopHeader > 0) {
      ...
      right.outerHeight() > initialLeftHeight &&
      remainingHeight > leftFainalHeight &&
      visibleTopHeader === 0
    ) {
      ...
      left.css(
        "top",
        `${
          bottomHeader.outerHeight() + $(".breadcrumb-nav").outerHeight() + 40
        }px`
      );
